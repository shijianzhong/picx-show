...
export default function CategoryGallery({ dir, categoryName }: CategoryGalleryProps) {
  ...
  const fetchImages = async (start = 0, append = false) => {
    try {
      const response = await fetch(`/api/images?dir=${dir}&start=${start}&count=20`)
    ...
    <div>
      ...
      <div className="mb-8 p-6 bg-white rounded-lg shadow-sm">
        ...
        <p className="text-gray-600">
          这里展示了精选的{categoryName}，所有图片都经过精心挑选，确保高质量。
          您可以浏览、收藏和下载喜欢的图片。
        </p>
        {images.length > 0 && (
